...
class TestPathUtilities:
    ...
    def test_is_within_boundary(self):
        ...
        assert (
            is_within_boundary(Path("/home/user/project/src/main.py"), boundary) is True
        )
    ...
    def test_is_within_any_boundary(self):
        ...
        assert (
            is_within_any_boundary(Path("/home/user/project/src"), boundaries) is True
        )
...
class TestPermissionEnforcer:
    ...
    def test_plan_mode_policy_blocks_non_read_operations(self):
        enforcer = PermissionEnforcer(mode=PermissionMode.FULL)
        enforcer.add_policy(AgentModePolicy())

        denied = enforcer.check(
            Operation.FILESYSTEM_WRITE,
            "/tmp/test.txt",
            context={"agent_mode": "plan"},
        )
        assert denied == PermissionResult.DENY
...
class TestToolPermissionMapping:
    ...
    def test_extract_resource(self):
        ...
        assert (
            extract_resource_from_input("read_file", {"path": "/test.py"}) == "/test.py"
        )
        assert (
            extract_resource_from_input("execute_command", {"command": "ls -la"})
            == "ls -la"
        )
        assert (
