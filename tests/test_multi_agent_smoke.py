...
class StubEngine:
    def __init__(self, conversation_manager: StubConversationManager) -> None:
        ...
        self.run_single_turn = AsyncMock(
            return_value={"assistant_response": "stub-response", "action_results": []}
        )
...
def _build_core_with_stubs(
    cm: StubConversationManager, engine: StubEngine
) -> PenguinCore:
...
def test_sub_agent_clamp_and_partial_share_once(tmp_path):
    ...
    child_context_messages = [
        msg.content
        for msg in child_conv.session.messages
...
async def test_core_process_routes_agent_context():
    ...
    assert engine.requested_agent == "planner"
    assert [conversation_id for _, conversation_id in stub_cm.loaded] == ["conv-123"]
    assert [path for _, path in stub_cm.loaded_files] == ["notes.md"]
    assert stub_cm.saved is True

    message_calls = [
        call for call in core.emit_ui_event.call_args_list if call.args[0] == "message"
    ]
