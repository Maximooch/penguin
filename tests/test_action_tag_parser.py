...
def test_action_type_coverage():
    ...
    for category, keywords in common_categories.items():
        matching = [
            ...
            if any(kw in a.value.lower() for kw in keywords)
        ]
...
def test_gateway_detection():
    ...
    config = ModelConfig(
        model="test-model", provider="openrouter", client_preference="openrouter"
    )
    ...
    test_contents = [
        ...
        "<replace_lines>file.py:1:2:new content</replace_lines>",
        "<insert_lines>file.py:2:more content</insert_lines>",
        "<delete_lines>file.py:1:2</delete_lines>",
    ...
    for content in test_contents:
        ...
        print(
            f"{content[:38]:<40} | {str(parser_found):<8} | {str(gateway_found):<8} | {match_symbol}"
        )
...
def test_edge_cases():
    ...
    config = ModelConfig(
        model="test", provider="openrouter", client_preference="openrouter"
    )
    gateway = OpenRouterGateway.__new__(OpenRouterGateway)
    ...
    for content, description in edge_cases:
        ...
        print(
            f"  {status} {description:<25} | Parser: {parser_found} | Gateway: {gateway_detected}"
        )
