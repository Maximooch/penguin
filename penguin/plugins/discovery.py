    ...
    def _discover_from_entry_points(self) -> None:
        ...
        try:
            ...
            if hasattr(discovered_entry_points, "select"):
                plugin_entry_points = discovered_entry_points.select(
                    group=self.entry_point_group
                )
            else:
                plugin_entry_points = discovered_entry_points.get(
            ...
            for entry_point in plugin_entry_points:
                try:
                    ...
                    if not module_name:
                        module_name = str(entry_point.value).split(":", 1)[0]
                    plugin_info = {
                    ...
                    logger.error(
                        f"Error processing entry point {entry_point.name}: {e}"
                    )
