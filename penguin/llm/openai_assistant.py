...
class OpenAIAssistantManager:
    ...
    def _initialize_assistant(self):
        try:
            ...
            if not self.model_config.model.startswith('gpt-4') and not self.model_config.model.startswith('gpt-3.5'):
                raise ValueError(f"Model {self.model_config.model} is not supported for Assistants API. Must be gpt-4* or gpt-3.5*")
            
            # Use configured assistant ID if available
            if hasattr(self.model_config, 'assistant_id') and self.model_config.assistant_id:
            ...
            try:
                assistants_page = get_assistants(
                ...
                for assistant in assistants:
                    if assistant.name.startswith('Penguin-'):
                        if not penguin_assistant or assistant.created_at > penguin_assistant.created_at:
                            penguin_assistant = assistant
                
                if penguin_assistant:
                    self.assistant_id = penguin_assistant.id
                    print(f"Using existing Penguin assistant with ID: {self.assistant_id}")
                else:
    ...
    def run_assistant(self):
        ...
        run = run_thread(
            ...
            instructions=self.current_instructions if hasattr(self, 'current_instructions') else None,
            custom_llm_provider='openai'
        )
        return run
