...
class LLMClientConfig:
    ...
    def from_env(cls) -> "LLMClientConfig":
        ...
        if link.is_configured:
            logger.info(
                f"Link integration configured: user_id={link.user_id}, base_url={base_url or 'default'}"
            )
        elif base_url:
    ...
    def is_link_proxy(self) -> bool:
        ...
        return (
            "link" in lower_url
            or "localhost:3001" in lower_url
            or "127.0.0.1:3001" in lower_url
        )
...
class LLMClient:
    ...
    def _get_gateway(self) -> Any:
        ...
        with self._gateway_lock:
            ...
            self._gateway = self._provider_registry.create_handler(
                self.model_config,
                base_url=base_url,
