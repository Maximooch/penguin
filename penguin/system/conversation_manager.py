...
class ConversationManager:
    ...
    def __init__(
        ...
        logger.info(
            f"Context window initialized with max tokens: {self.context_window.max_context_window_tokens}"
        )
        ...
        logger.info(
            f"Session manager initialized with {len(self.session_manager.session_index)} sessions"
        )
        ...
        if checkpoint_config.enabled:
            try:
                ...
                logger.info(
                    f"Checkpoint manager initialized with {len(self.checkpoint_manager.checkpoint_index)} checkpoints"
                )
        ...
        self.context_loader = SimpleContextLoader(context_manager=self.conversation)
        ...
        try:
            autoload = bool(
                global_config.get("context", {}).get("autoload_project_docs", True)
            )
        except Exception:
            autoload = True
        if autoload and self.context_window:
            try:
                content, info = self.context_window.load_project_instructions(
                ...
                if content:
                    ...
                    logger.info(
                        f"Project docs autoloaded: {', '.join(info.get('loaded_files', []))}"
                    )
            except Exception as e:
        ...
        # -----------------------------------------------------------
        try:
            from penguin.system.snapshot_manager import (
                SnapshotManager,
            )  # local import to avoid circulars

            snapshots_path = Path(self.workspace_path) / "snapshots" / "snapshots.db"
            self.snapshot_manager: Optional[SnapshotManager] = SnapshotManager(
                snapshots_path
            )
        except Exception as e:
            logger.warning(
                f"Failed to initialise SnapshotManager – snapshot/restore disabled: {e}"
            )
        ...
        self.agent_sessions: Dict[str, ConversationSystem] = {
            "default": self.conversation
        }
        self.agent_session_managers: Dict[str, SessionManager] = {
            "default": self.session_manager
        }
        # Namespace checkpoint managers per agent (optional; default agent uses the prebuilt one).
        self.agent_checkpoint_managers: Dict[str, Optional[CheckpointManager]] = {
            "default": self.checkpoint_manager
        }
        # Separate context windows per agent (default shares existing)
        self.agent_context_windows: Dict[str, ContextWindowManager] = {
    ...
    def _get_live_config(self):
        ...
        try:
            ...
            if (
                hasattr(self, "core")
                and getattr(self, "core")
                and hasattr(self.core, "config")
            ):
    ...
    def _ensure_agent(self, agent_id: str) -> None:
        ...
        if self.checkpoint_manager is not None:
            try:
                ...
                logger.warning(
                    f"Failed to init agent checkpoint manager for '{agent_id}': {e}"
                )
                agent_cp = None
    ...
    def get_current_context_window(self) -> Optional[ContextWindowManager]:
        ...
        try:
            return self.agent_context_windows.get(
                self.current_agent_id, self.context_window
            )
        except Exception:
    ...
    def create_sub_agent(
        ...
        *,
        ...
        if share_session:
            ...
            self.agent_session_managers[agent_id] = self.agent_session_managers[
                parent_agent_id
            ]
            self.agent_checkpoint_managers[agent_id] = (
                self.agent_checkpoint_managers.get(parent_agent_id)
            )
        else:
            self._ensure_agent(agent_id)

        if share_context_window:
            self.agent_context_windows[agent_id] = self.agent_context_windows[
                parent_agent_id
            ]
        ...
        if not share_context_window:
            try:
                ...
                if (
                    parent_cw
                    and child_cw
                    and hasattr(parent_cw, "max_context_window_tokens")
                    and hasattr(child_cw, "max_context_window_tokens")
                ):
                    ...
                    if shared_context_window_max_tokens is not None:
                        note = f"Note: Sub-agent '{agent_id}' uses isolated CWM with max_context_window_tokens={desired} (parent={parent_cw.max_context_window_tokens})."
                        meta = {
                        ...
                        self.add_system_note(
                            parent_agent_id, content=note, metadata=dict(meta)
                        )
                        self.add_system_note(
                            agent_id,
                            content=note,
                            metadata={**meta, "audience": "child"},
                        )
    ...
    def partial_share_context(
        ...
        *,
        ...
        if replace_child_context:
            try:
                child_conv.session.messages = [
                    m for m in child_conv.session.messages if m.category not in cats
                ]
                child_conv._modified = True
            except Exception:
                pass
    ...
    def list_all_conversations(
        self, *, limit_per_agent: int = 1000, offset: int = 0
    ) -> List[Dict[str, Any]]:
    ...
    def log_delegation_event(
        ...
        *,
        ...
        content = (
            message
            or f"Delegation {delegation_id} ({parent_agent_id} → {child_agent_id}): {event}"
        )
        ...
        if mirror_to_child:
            try:
                ...
                logger.debug(
                    "Failed to mirror delegation event to child '%s'", child_agent_id
                )
    ...
    def agents_sharing_session(self, agent_id: str) -> List[str]:
        ...
        shared = [
            aid for aid, conv in self.agent_sessions.items() if conv is target_conv
        ]
        return shared
        ...
        try:
            ...
            if conversation_id:
                if not self.load(conversation_id):
                    logger.warning(
                        f"Failed to load conversation {conversation_id}, creating new one"
                    )
            ...
            if streaming and self.api_client:
                if hasattr(self.api_client, "get_streaming_response"):
                    ...
                    async def stream_generator():
                        async for chunk in self.api_client.get_streaming_response(
                            messages=messages, stream_callback=stream_callback
                        ):
                            yield chunk
                            last_chunk = chunk
                    ...
                    logger.warning(
                        "API client doesn't support streaming, falling back to non-streaming"
                    )
                    streaming = False
            else:
                # Non-streaming path
                if self.api_client:
                    ...
                    if session_id in self.session_manager.session_index:
                        self.session_manager.session_index[session_id][
                            "token_count"
                        ] = session.total_tokens
                        self.session_manager._save_index(
                            self.session_manager.session_index
                        )
    ...
    def reset(self) -> None:
        ...
        try:
            self.conversation.session.metadata["agent_id"] = getattr(
        ...
        for conversation in conversations:
            ...
            if session_id:
                conversation.setdefault(
                    ...
                    or conversation.get("metadata", {}).get("agent_id"),
                )
            if session_id and not conversation.get("title"):
                ...
                try:
                    ...
                    if session_id in self.session_manager.sessions:
                        ...
                        session_path = (
                            self.session_manager.base_path
                            / f"{session_id}.{self.session_manager.format}"
                        )
                    ...
                    logger.warning(
                        f"Error extracting title for session {session_id}: {e}"
                    )
        ...
        *,
        ...
        for message in session.messages:
            ...
            entry = {
                ...
                if hasattr(message.category, "name")
                else message.category,
    ...
    def delete_conversation(self, conversation_id: str) -> bool:
        ...
        if (
            ...
            and self.conversation.session.id == conversation_id
        ):
            self.reset()
    ...
    def create_snapshot(
        ...
        snap_id = self.snapshot_manager.snapshot(
            payload, parent_id=parent_id, meta=meta
        )
    ...
    def get_context_sharing_info(self, agent_id: str) -> Dict[str, Any]:
        ...
        result["shares_with_children"] = [
            child for child in children if self.shares_context_window(agent_id, child)
        ]
    ...
    def sync_context_to_child(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                f"Failed to sync context from '{parent_agent_id}' to '{child_agent_id}': {e}"
            )
            return False
    ...
    def get_shared_context_agents(self, agent_id: str) -> List[str]:
        ...
        return [
            aid
            for aid, other_cw in self.agent_context_windows.items()
    ...
    def remove_agent(self, agent_id: str) -> bool:
        ...
        if (
            getattr(self, "context_loader", None)
            and getattr(self.context_loader, "context_manager", None)
            is self.conversation
        ):
    ...
    async def branch_from_checkpoint(
        ...
        if branch_id:
            ...
            if self.session_manager.current_session is not None:
                self.session_manager.mark_session_modified(
