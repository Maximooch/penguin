    ...
    try:
        with Progress(
            ...
            ollama_task = progress.add_task(
                "[yellow]Starting Ollama server...", total=None
            )
            memory_task = progress.add_task(
                "[yellow]Initializing memory search...", total=None
            )
            indexer_task = progress.add_task(
                "[yellow]Indexing workspace...", total=None
            )
            ...
            penguin_core = PenguinCore(api_client=api_client, tool_manager=tool_manager)
            ...
            for coro, task_id in init_tasks:
                try:
                    ...
                    progress.update(
                        task_id,
                        description=f"[green]✓ {progress.tasks[task_id].description[8:]}",
                    )
                except asyncio.TimeoutError:
                    progress.update(
                        task_id,
                        description=f"[red]⨯ {progress.tasks[task_id].description[8:]} (timeout)",
                    )
                except Exception as e:
                    progress.update(
                        task_id,
                        description=f"[red]⨯ {progress.tasks[task_id].description[8:]} ({str(e)})",
                    )
...
async def main():
    ...
    console.print(
        f"[yellow]Workspace initialization: {workspace_end - workspace_start:.2f}s[/yellow]"
    )
