...
class ProjectVisualizer:
    ...
    def create_dashboard(self, projects: list, save_name: str = "project_stats.png") -> Path:
        ...
        if dates:
            scatter = ax4.scatter(dates, progresses, alpha=0.6, 
                                c=progresses, cmap='RdYlGn')
            ...
            for i, title in enumerate(titles):
                ax4.annotate(title, (dates[i], progresses[i]),
    ...
    def show_terminal_charts(self, projects: list) -> None:
        ...
        for project in projects:
            ...
            tasks = sorted(self._get_tasks(project), 
            ...
            colors = ['red' if p < 30 else 'yellow' if p < 70 else 'green' 
                     for p in progress]
    ...
    def create_gantt_chart(self, projects: list, save_name: str = "gantt.png") -> Path:
        ...
        for idx, task in enumerate(tasks):
            ...
            ax.barh(idx, duration.total_seconds(), 
                   left=task['start'], alpha=0.3,
                   color=['red', 'orange', 'green'][task['priority']-1])
            
            # Draw progress overlay
            if task['progress'] > 0:
                ax.barh(idx, (progress_date - task['start']).total_seconds(),
                       left=task['start'], alpha=0.8,
