...
def write_to_file(path, content):
    ...
    for encoding in encodings:
        try:
            ...
            logging.getLogger(__name__).debug(
                f"Error with encoding '{encoding}': {str(e)}"
            )
            continue
            ...
            try:
                ...
                if removed_funcs:
                    semantic_info.append(
                        f"Removed functions: {', '.join(removed_funcs)}"
                    )
                if added_classes:
                    semantic_info.append(f"Added classes: {', '.join(added_classes)}")
                if removed_classes:
                    semantic_info.append(
                        f"Removed classes: {', '.join(removed_classes)}"
                    )
...
def find_files_enhanced(
    pattern, search_path=".", include_hidden=False, file_type=None, workspace_path=None
):
    """
    ...
    try:
        ...
        # Log for debugging (not shown to user)
        logging.getLogger(__name__).debug(
            f"Searching for '{pattern}' in: {target_path}"
        )
...
def analyze_project_structure(
    ...
    try:
        ...
        # Log for debugging (not shown to user)
        logging.getLogger(__name__).debug(
            f"Analyzing project structure in: {target_path}"
        )
        ...
        sorted_files = sorted(
            file_stats.items(), key=lambda x: x[1]["lines"], reverse=True
        )

        for file_path, stats in sorted_files[:10]:  # Show top 10
            result.append(
                f"  {file_path}: {stats['lines']} lines, {stats['functions']} functions, {stats['classes']} classes"
            )
...
def _strip_diff_fences(diff_content: str) -> str:
    """Remove surrounding ``` fences from diff content if present."""
    if not diff_content:
        return diff_content
...
def _build_single_file_patch(
    file_path: str, diff_content: str, workspace_path: str | None
) -> str:
    """Ensure unified diff has headers for a single file patch."""
    lines = diff_content.splitlines()
    if any(line.startswith("--- ") for line in lines):
        return diff_content if diff_content.endswith("\n") else diff_content + "\n"

    rel_path = file_path
    try:
        path_obj = Path(file_path)
        if path_obj.is_absolute() and workspace_path:
            rel_path = str(
                path_obj.resolve().relative_to(Path(workspace_path).resolve())
            )
    except Exception:
        rel_path = file_path

    rel_path = _strip_prefix(rel_path)
    patch = [f"--- a/{rel_path}", f"+++ b/{rel_path}"]
    patch.append(diff_content.lstrip("\n"))
    return "\n".join(patch) + "\n"
    ...
    try:
        ...
        try:
            ...
            if isinstance(modified_content, dict) and modified_content.get("error"):
                ...
                if allow_fallback:
                    try:
                        patch_text = _build_single_file_patch(
                            file_path, diff_content, workspace_path
                        )
                        fallback_result = apply_unified_patch(
                            patch_text,
                        ...
                        if return_json:
                            ...
                            fallback_result, str
                        ) and not fallback_result.lower().startswith("error"):
                            return fallback_result
                    except Exception:
                        fallback_result = None
                ...
                err_msg += (
                    f" (hunks={analysis['hunks']}, adds={analysis['adds']}, "
                    f"dels={analysis['dels']}, ctx={analysis['context']})"
                )
                if log_path:
...
def _apply_unified_diff(original_content, diff_content):
    ...
    while idx < len(diff_lines):
        ...
        if not header.startswith("@@"):
            ...
            continue
        m = re.match(
            r"@@ -(?P<o_start>\d+)(?:,(?P<o_cnt>\d+))? \+(?P<n_start>\d+)(?:,(?P<n_cnt>\d+))? @@",
            header,
        )
        if not m:
    ...
    for o_start, o_cnt, n_cnt, ops in reversed(hunks):
        ...
        if old_norm != expected_norm:
            ...
            if old_norm != expected_norm:
                return {
                    "error": "Context mismatch while applying diff (file changed since diff was generated)"
                }
...
def _log_diff_failure(
    file_path: str, diff_content: str, original_content: str, workspace_path: str | None
) -> str | None:
    ...
    try:
        ...
        (dirp / "target.txt").write_text(
            original_content, encoding="utf-8", errors="ignore"
        )
        (dirp / "patch.diff").write_text(
            diff_content, encoding="utf-8", errors="ignore"
        )
        (dirp / "file_path.txt").write_text(
            str(file_path), encoding="utf-8", errors="ignore"
        )
        return str(dirp)
    except Exception:
        return None
    ...
    while i < len(lines):
        ...
        if line.startswith("--- "):
            ...
            if buf and from_path is not None and to_path is not None:
                parts.append(
                    {"from": from_path, "to": to_path, "content": "\n".join(buf) + "\n"}
                )
                buf = []
                from_path = to_path = None
...
def _normalize_unified_patch_paths(file_patches: list[dict], base: Path) -> str:
    ...
    for fp in file_patches:
        ...
        if raw_from in ("/dev/null", "dev/null", ""):
            ...
            try:
                rel_from = (
                    str(p_from.relative_to(base)) if p_from.is_absolute() else raw_from
                )
            except Exception:
                rel_from = raw_from
...
def apply_unified_patch(
    ...
    use_robust = os.environ.get("PENGUIN_PATCH_ROBUST", "").lower() in (
        "1",
        "true",
        "yes",
        "on",
    )
    three_way = os.environ.get("PENGUIN_PATCH_THREEWAY", "").lower() in (
        "1",
        "true",
        "yes",
        "on",
    )
    use_shadow = os.environ.get("PENGUIN_PATCH_SHADOW", "").lower() in (
        "1",
        "true",
        "yes",
        "on",
    ...
    if robust_ok:
        try:
            ...
            for fp in file_patches:
                ...
                _ = enforce_allowed_path(
                    target, root_pref=root_pref, cwd_override=str(base)
                )
        except Exception:
            robust_ok = False
                            ...
                            return json.dumps(
                                {
                                    ...
                                    "message": app.stderr.strip()
                                    or app.stdout.strip()
                                    or "git apply --3way produced conflicts",
                                }
                            )
                        return (
                        ...
                        if not existed and p.exists():
                            ...
                            if bak.exists():
                                try:
                                    ...
                                    pass
                    log_path = _log_diff_failure(
                        "<git-apply-3way>", patch_text, "", str(base)
                    )
                    err = (
                        app.stderr.strip()
                        or app.stdout.strip()
                            ...
                            return json.dumps(
                                {
                                    ...
                                    "message": app.stderr.strip()
                                    or app.stdout.strip()
                                    or "git apply produced conflicts",
                                }
                            )
                        return (
    ...
    if robust_ok and use_shadow:
        try:
            ...
            default_branch = (
                f"penguin/checkpoints/waddle-{_dt.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            files = [
                str((shadow_root / f.strip()).resolve())
    ...
    for fp in file_patches:
        ...
        if is_new_file_patch and not target.exists():
            ...
            try:
                ...
                target = enforce_allowed_path(
                    target, root_pref=root_pref, cwd_override=workspace_path
                )
...
def edit_file_with_pattern(
    ...
    try:
        ...
        # Log for debugging (not shown to user)
        logging.getLogger(__name__).debug(
            f"Editing file with pattern replacement: {target_path}"
        )
        ...
        logging.getLogger(__name__).debug(
            f"Pattern replacement applied to: {target_path}"
        )
...
def edit_file_at_line(
    ...
    try:
        ...
        # Log for debugging (not shown to user)
        logging.getLogger(__name__).debug(
            f"Editing file at line {line_number}: {target_path}"
        )
        ...
        logging.getLogger(__name__).debug(
            f"Line {line_number} operation '{operation}' applied to: {target_path}"
        )
        ...
        for encoding in encodings:
            try:
                if target_path.exists():
                    ...
                    logging.getLogger(__name__).debug(
                        f"New file created: {target_path}"
                    )
...
def enhanced_read_file(
    path, show_line_numbers=False, max_lines=None, workspace_path=None
):
    """
    ...
    try:
        ...
        for encoding in encodings:
            try:
                ...
                logging.getLogger(__name__).debug(
                    f"File read successfully: {target_path} ({len(content)} characters)"
                )
                return content
