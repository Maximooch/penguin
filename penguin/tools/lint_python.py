...
def lint_python(target: str, is_file: bool) -> dict:
    try:
        ...
        # Run Flake8
        flake8_result = subprocess.run(['flake8', str(file_path)], capture_output=True, text=True)
        if flake8_result.stdout:
            results.append(("Flake8 (Style)", flake8_result.stdout))

        # Run Pylint
        pylint_result = subprocess.run(['pylint', str(file_path)], capture_output=True, text=True)
        if pylint_result.stdout:
            results.append(("Pylint (Code Quality)", pylint_result.stdout))

        # Run mypy
        mypy_result = subprocess.run(['mypy', str(file_path), '--strict'], capture_output=True, text=True)
        if mypy_result.stdout:
            results.append(("mypy (Type Checking)", mypy_result.stdout))

        # Run Bandit
        bandit_result = subprocess.run(['bandit', '-r', str(file_path)], capture_output=True, text=True)
