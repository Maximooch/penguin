...
class ToolManager:
        ...
        with profile_startup_phase("ToolManager.__init__"):
            ...
            if project_root_env:
                try:
                    ...
                    logger.info(
                        "ToolManager env override project_root=%s", self.project_root
                    )
                except Exception:
                    logger.warning(
                        "Invalid PENGUIN_PROJECT_ROOT=%s; falling back to auto-detect",
                        project_root_env,
                    )
            ...
            try:
                if hasattr(config, "diagnostics") and hasattr(
                    ...
                    # Dict-style config
                    self.diagnostics_enabled = config.get("diagnostics", {}).get(
                        "enabled", False
                    )
                else:
                    # Fallback
            ...
            logger.info(
                f"ToolManager initialized with true lazy loading. Fast startup: {fast_startup}"
            )
            ...
            },
            {
                ...
                "input_schema": {
                    ...
                    "properties": {
                        ...
                        "analysis_type": {
                            "type": "string",
                            "enum": ["dependencies", "complexity", "patterns", "all"],
                            "default": "all",
                        },
    ...
    async def _background_initialize_async_components(self):
        ...
        try:
            with profile_operation("ToolManager.background_memory_init"):
                logger.info(
                    "Background: Initializing memory provider and starting indexing..."
                )
                await self._ensure_memory_provider_and_index()
                self._indexing_completed = True
                logger.info(
                    "Background: Memory provider initialization and indexing completed"
                )
        except Exception as e:
    ...
    async def _ensure_memory_provider_and_index(self):
        ...
        try:
            with profile_operation("ToolManager.incremental_indexing"):
                ...
                try:
                    if hasattr(self.config, "get"):
                        ...
                        config_dict = self.config.__dict__
                        workspace_path = Path(
                            config_dict.get("workspace", {}).get("path", ".")
                        )
                    else:
                        workspace_path = Path(".")
                except Exception:
            ...
            logger.error(
                f"IncrementalIndexer failed: {e}. Falling back to simple indexing."
            )
            with profile_operation("ToolManager.fallback_indexing"):
                await self._initial_indexing()

    def _initialize_memory_provider(
    ...
    def grep_search(self):
        if not self._lazy_initialized["grep_search"]:
            with profile_operation("ToolManager.lazy_load_grep_search"):
                ...
                self._grep_search = GrepSearch(
                    root_dir=os.path.join(WORKSPACE_PATH, "logs")
                )
    ...
    def summary_notes_tool(self):
        if not self._lazy_initialized["summary_notes_tool"]:
            with profile_operation("ToolManager.lazy_load_summary_notes_tool"):
                ...
                summary_notes_path = os.path.join(
                    WORKSPACE_PATH, "notes", "summary_notes.yml"
                )
        ...
        if self._permission_enforcer is None and self._permission_enabled:
            ...
            if _PermissionEnforcer is not None:
                with profile_operation("ToolManager.lazy_load_permission_enforcer"):
                    ...
                    yolo = os.environ.get("PENGUIN_YOLO", "").lower() in (
                        "1",
                        "true",
                        "yes",
                    )
        ...
        if not self._lazy_initialized["memory_provider"]:
            with profile_operation("ToolManager.lazy_load_memory_provider"):
                ...
                if not self._memory_provider:
                    ...
                    self._memory_provider = self._initialize_memory_provider(
                        memory_config
                    )
                ...
                if self._memory_provider:
                    ...
                    if not self._indexing_completed and not self._indexing_task:
                        try:
                            ...
                            self._indexing_task = loop.create_task(
                                self._background_initialize_async_components()
                            )
                            logger.info(
                                "Started background indexing task (lazy loaded)"
                            )
                        except RuntimeError:
    ...
    def code_indexer(self):
        ...
        raise NotImplementedError(
            "Code indexer is currently disabled for performance reasons."
        )
    ...
    def memory_searcher(self):
        ...
        raise NotImplementedError(
            "Memory searcher is currently disabled. Use memory_search via ToolManager instead."
        )
    ...
    def _execute_enhanced_diff(self, tool_input: dict) -> str:
        ...
        if result_container["error"] is not None:
            return json.dumps(
                {"error": result_container["error"], "tool": "enhanced_diff"}
            )
    ...
    def _execute_analyze_project(self, tool_input: dict) -> str:
        ...
        if result_container["error"] is not None:
            return json.dumps(
                {"error": result_container["error"], "tool": "analyze_project"}
            )
        return result_container["result"]
    ...
    def set_execution_root(self, mode: str) -> str:
        ...
        self._file_root = (
            self.project_root if mode_l == "project" else self.workspace_root
        )
        logger.info(
    ...
    def on_runtime_config_change(self, config_key: str, new_value: Any) -> None:
        ...
        if config_key == "project_root":
            ...
            if self.file_root_mode == "project":
                ...
                logger.info(
                    f"ToolManager: Updated file_root to {self._file_root} (project mode)"
                )
            ...
            if self.file_root_mode == "workspace":
                ...
                self._refresh_file_map()
                logger.info(
                    f"ToolManager: Updated file_root to {self._file_root} (workspace mode)"
                )
            ...
            if mode_lower in ("project", "workspace"):
                ...
                self._file_root = (
                    self.project_root
                    if mode_lower == "project"
                    else self.workspace_root
                )
                self._refresh_file_map()
                logger.info(
                    f"ToolManager: Switched to {mode_lower} mode, file_root={self._file_root}"
                )

                # Update notebook executor directory if loaded
                if (
                    self._lazy_initialized.get("notebook_executor")
                    and self._notebook_executor
                ):
                    try:
                        self._notebook_executor.active_directory = self._file_root
                    except Exception as e:
                        logger.warning(
                            f"Could not update notebook executor directory: {e}"
                        )
        ...
        with profile_operation(f"ToolManager.execute_tool.{tool_name}"):
            ...
            tool_map = {
                ...
                "browser_navigate": lambda: self._execute_async_tool(
                    self.execute_browser_navigate(tool_input["url"])
                ),
                "browser_interact": lambda: self._execute_async_tool(
                    self.execute_browser_interact(
                        tool_input["action"],
                        tool_input["selector"],
                        tool_input.get("text"),
                    )
                ),
                "browser_screenshot": lambda: self._execute_async_tool(
                    self.execute_browser_screenshot()
                ),
                "pydoll_browser_navigate": lambda: self._execute_async_tool(
                    self.execute_pydoll_browser_navigate(tool_input["url"])
                ),
                "pydoll_browser_interact": lambda: self._execute_async_tool(
                    self.execute_pydoll_browser_interact(
                        tool_input["action"],
                        tool_input["selector"],
            ...
            if tool_name not in tool_map:
                ...
                self.log_error(
                    ...
                    f"Attempted to use unknown tool: {tool_name}",
                )
    ...
    def execute_code(self, code: str) -> str:
        ...
        try:
            default_timeout = int(
                os.environ.get(
                    "PENGUIN_TOOL_TIMEOUT_CODE",
                    os.environ.get("PENGUIN_TOOL_TIMEOUT", "300"),
                )
            )
        except Exception:
            default_timeout = 300
    ...
    async def execute_browser_interact(
        self, action: str, selector: str, text: Optional[str] = None
    ) -> str:
    ...
    async def execute_with_screenshot_on_error(
        self, coroutine, description="browser action"
    ):
            ...
            try:
                ...
                if page:
                    ...
                    error_path = os.path.join(
                        os.getcwd(), "error_screenshots", f"error_{timestamp}.png"
                    )
                ...
                logging.error(
                    f"Failed to capture error screenshot: {str(screenshot_e)}"
                )
    ...
    async def execute_pydoll_browser_interact(
        self,
        action: str,
        selector: str,
        ...
        try:
            result = await self.pydoll_browser_interaction_tool.execute(
                action, selector, selector_type, text
            )
            return result
        except Exception as e:
    ...
    def perform_memory_search_sync(
        ...
        try:
            ...
            try:
                ...
                return asyncio.run(
                    self.perform_memory_search(query, k, memory_type, categories)
                )
            ...
            return json.dumps(
                {
                    "error": "Memory search from sync context within async environment not yet supported. Use ActionExecutor instead."
                }
            )
        ...
        try:
            ...
            try:
                if hasattr(self.config, "get"):
                    ...
                    allow_memory_tools = (
                        tools_config.get("allow_memory_tools", False)
            ...
            if not memory_config:
                return json.dumps(
                    {"error": "Memory system is not configured in config.yml."}
                )
            ...
            results = await memory_provider.search_memory(
                query, max_results=k, filters=filters
            )
    ...
    def analyze_codebase(
        ...
        try:
            if hasattr(self.config, "get"):
                allow_memory_tools = self.config.get("tools", {}).get(
        ...
        if not allow_memory_tools:
            return json.dumps(
                {"error": "Code analysis tools are disabled in config.yml."}
            )
        ...
        try:
            ...
            if not target_dir.exists():
                return json.dumps(
                    {"error": f"Directory '{target_dir}' does not exist."}
                )
            ...
            if analysis_type in ["all", "dependencies"]:
                analysis_results["dependencies"] = {
                    "import_count": sum(
                        len(imports) for imports in all_imports.values()
                    ),
                    "dependency_graph": {
            ...
            if analysis_type in ["all", "complexity"]:
                analysis_results["complexity"] = {
                    **complexity_metrics,
                    "avg_functions_per_file": complexity_metrics["total_functions"]
                    / max(analysis_results["files_analyzed"], 1),
                    "avg_classes_per_file": complexity_metrics["total_classes"]
                    / max(analysis_results["files_analyzed"], 1),
                    "avg_lines_per_file": complexity_metrics["total_lines"]
                    / max(analysis_results["files_analyzed"], 1),
                }
            ...
            if analysis_type in ["all", "patterns"]:
                analysis_results["patterns"] = {
                    ...
                    "documented_functions": len(
                        [f for f in all_functions if f["docstring"]]
                    ),
                    "documented_classes": len(
                        [c for c in all_classes if c["docstring"]]
                    ),
                    "largest_classes": sorted(
                        all_classes, key=lambda x: x["method_count"], reverse=True
                    )[:5],
    ...
    def _get_most_common_imports(self, all_imports: dict) -> list:
        ...
        return [
            ...
            for module, count in counter.most_common(10)
        ]
    ...
    async def reindex_workspace(
        ...
        try:
            ...
            indexable_files = [
                ...
                and not any(
                    ignore in str(f)
                    for ignore in [".git", "__pycache__", ".DS_Store", "node_modules"]
                )
            ]

            logger.info(
                f"Reindexing {len(indexable_files)} files (force_full={force_full})"
            )
            ...
            for i in range(0, len(indexable_files), batch_size):
                ...
                for file_path in batch:
                    ...
                    batch_tasks.append(
                        self._index_single_file(file_path, stats, memory_provider)
                    )
            ...
            logger.info(
                f"Workspace reindexing completed in {elapsed:.2f}s. Files: {stats['files_processed']} processed, {stats['files_skipped']} skipped, {stats['files_failed']} failed"
            )
    ...
    async def _index_single_file(
        self, file_path: Path, stats: dict, memory_provider: MemoryProvider
    ) -> None:
    ...
    async def _index_code_file(
        self, file_path: Path, memory_provider: MemoryProvider
    ) -> None:
            ...
            for file_path in directory.rglob("*"):
                ...
                try:
                    if (
                        ...
                        await memory_provider.add_memory(
                            content=content, metadata=metadata, categories=[category]
                        )
                        files_indexed += 1
                except Exception as e:
    ...
    async def _index_conversation_file(
        self, file_path: Path, memory_provider: MemoryProvider
    ):
        ...
        try:
            ...
            await memory_provider.add_memory(
                content=content, metadata=metadata, categories=["conversations"]
            )
