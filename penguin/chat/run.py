...
def handle_image_input(chat_manager, message_count, log_file):
...
def handle_automode(chat_manager, user_input, message_count, log_file):
    try:
...
def run_automode(chat_manager, automode_goal, max_iterations, message_count, log_file):
    for iteration in range(max_iterations):
        try:
            response, exit_continuation = chat_manager.chat_with_claude(automode_goal, message_count, current_iteration=iteration+1, max_iterations=max_iterations)
            ...
            message_count += 1
            automode_goal = "Continue with the next step. Or STOP by saying 'AUTOMODE_COMPLETE' if you think you've achieved the results established in the original request."
...
def handle_automode_interruption(chat_manager, log_file, message_count):
