...
#             # If the last message was from the user, we need to add a placeholder assistant message
#             self.memory.add_assistant_message("Continuing the conversation...")
...
#             self.memory.add_user_message(f"User input for image: {user_input}", image_data=image_base64)
#             self.print_colored("Image message added to conversation history", TOOL_COLOR)
#         else:
#             self.memory.add_user_message(user_input)
...
#         assistant_response, tool_uses = self.api_client.process_response(response)
...
#             self.memory.add_tool_message(tool_name, tool_input, result)
        
#         # If there were tool uses, we need to get a final response that takes into account the tool results
#         if tool_uses:
#             try:
#                 final_response = self.api_client.create_message(
#                     model=self.api_client.model_config.model,
...
#                     tool_choice={"type": "auto"}
#                 )
#                 assistant_response, _ = self.api_client.process_response(final_response)
#             except Exception as e:
#                 self.print_colored(f"Error in final response: {str(e)}", TOOL_COLOR)
#                 assistant_response += "\nI encountered an error while processing the tool results. Please try again."
...
#         return self.api_client.encode_image_to_base64(image_path)