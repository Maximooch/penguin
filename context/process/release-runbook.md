...
This runbook assumes the target release is `v0.6.0`. Replace the version string if
 needed.
...
pip install "/path/to/penguin_ai-0.6.0-py3-none-any.whl[tui]"
```

If shell extra syntax is awkward with a wheel path, use:

```bash
pip install "/path/to/penguin_ai-0.6.0-py3-none-any.whl"
...
source .venv/bin/activate
python -m pip install --upgrade pip
pip install "/absolute/path/to/dist/penguin_ai-0.6.0-py3-none-any.whl[tui]"
...
- release assets are attached to the same `v0.6.0` GitHub release
...
gh release download v0.6.0 --dir /tmp/penguin-release-assets --pattern "*.zip"
gh release download v0.6.0 --dir /tmp/penguin-release-assets --pattern "*.tar.gz"
...
- confirm the GitHub release tag exists: `v0.6.0`
...
- `v0.6.0` tag publishes both Python package and matching sidecar assets
