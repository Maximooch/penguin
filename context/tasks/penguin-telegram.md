...
- [ ] Extract the reusable orchestration in
  `penguin/web/routes.py::handle_chat_message` into a typed application service.
- [ ] Preserve execution context, session binding, request gate, model/variant,
  media normalization, permission policy, cancellation, and cleanup behavior.
- [ ] Migrate the HTTP route to the new service without changing its contract.
- [ ] Audit the WebSocket chat path and route it through the same service where
  feasible.
- [ ] Define `ChannelAddress`, `InboundEnvelope`, attachments, stream updates,
  and delivery requests.
- [ ] Add a minimal in-memory fake channel adapter for contract tests.
- [ ] Verify one direct service call can stream and complete a Penguin response.
- [ ] Verify same-session calls serialize and separate-session calls isolate.
- [ ] Decide and test the supported `python-telegram-bot` version across Penguin's
...
- [ ] Add the optional `telegram` dependency extra.
- [ ] Add typed disabled-by-default Telegram configuration.
- [ ] Create and stop the Telegram manager from the application lifespan.
- [ ] Support long polling for one bot token.
- [ ] Validate the token and expose bot identity/status without exposing it.
- [ ] Enforce a numeric DM allowlist before admitting work.
- [ ] Normalize text updates into `InboundEnvelope`.
- [ ] Persist a stable DM-to-Penguin session binding.
- [ ] Route text through the shared chat service.
- [ ] Show a typing indicator while Penguin runs.
- [ ] Send final output with conservative 4,000-character chunking.
- [ ] Render Telegram HTML with plain-text fallback.
- [ ] Add `/start`, `/help`, `/new`, `/status`, `/stop`, and `/whoami`.
- [ ] Handle provider/tool/runtime failures as errors, not assistant success text.
- [ ] Add update-ID deduplication before exposing the bot beyond local testing.
...
- [ ] Add throttled editable answer previews.
- [ ] Add `progress` mode with one status/tool message and a clean final answer.
- [ ] Keep reasoning hidden by default; expose only through explicit safe config.
- [ ] Coalesce small deltas and limit edit frequency.
- [ ] Rotate/chunk previews that exceed Telegram limits.
- [ ] Fall back from edit failure to ordinary final delivery.
- [ ] Register a native Telegram command menu.
- [ ] Add `/model`, `/mode`, `/session`, `/goal`, and `/project` where Penguin has
  stable service contracts for them.
- [ ] Download inbound photos into validated temporary files and pass them as
  Penguin image inputs.
- [ ] Ingest bounded UTF-8 text documents as context.
- [ ] Add a first-class outbound artifact projection for Penguin-generated images
  and files.
- [ ] Preserve reply-to context for observed messages.
- [ ] Clean all temporary media on success, failure, and cancellation.
...
- [ ] Add DM policies: pairing, allowlist, open, and disabled.
- [ ] Add one-hour, single-use pairing codes and approval CLI/API.
- [ ] Keep pairing grants DM-only.
- [ ] Add group allowlist and group-sender allowlist policy.
- [ ] Default groups to mention-gated behavior.
- [ ] Handle BotFather privacy-mode limitations clearly in diagnostics/docs.
- [ ] Ignore commands addressed to another bot.
- [ ] Create separate session bindings for groups and forum topics.
- [ ] Preserve topic IDs on typing, replies, edits, and final sends.
- [ ] Add bounded rolling group context for observed messages.
- [ ] Support per-group/topic prompt, directory, agent, mode, and skill binding.
- [ ] Add `/activation mention|always` and `/topic` behavior where appropriate.
- [ ] Handle supergroup ID migration without losing bindings.
...
- [ ] Project `QuestionManager` prompts as Telegram messages with inline choices.
- [ ] Route free-text question replies directly to the pending waiter.
- [ ] Ensure question replies do not deadlock behind the session request gate.
- [ ] Add callback ownership checks and single-use callback records.
- [ ] Project tool approval requests with approve/deny buttons.
- [ ] Add session/request/tool identity to approval callback payload records.
- [ ] Implement resumable approval semantics so approval can continue or retry the
  original tool operation safely.
- [ ] Expire stale questions/approvals and update their Telegram messages.
- [ ] Add callback unregister/cleanup behavior for application shutdown.
- [ ] Prevent approval command text from leaking into unauthorized groups/topics.
...
- [ ] Implement the SQLite channel session-binding store.
- [ ] Implement durable ingress admission, claims, leases, retry, and dead letters.
- [ ] Persist updates before advancing the polling watermark.
- [ ] Implement per-chat/topic lane scheduling.
- [ ] Implement the durable outbound delivery store and bounded workers.
- [ ] Classify Telegram retryable, fatal, rate-limit, and polling-conflict errors.
- [ ] Honor `retry_after` and use bounded jittered backoff.
- [ ] Recover expired ingress and delivery leases on startup.
- [ ] Add dead-letter list, inspect, retry, and discard operator controls.
- [ ] Detect duplicate token/poller ownership.
- [ ] Add authenticated webhook mode using the same ingress path as polling.
- [ ] Return webhook success only after durable adoption.
- [ ] Add webhook body limits, timeouts, and constant-time secret verification.
- [ ] Define crash behavior for turns that may have started non-idempotent tools.
